...
//   const allTasks = getTask('all');
//   const pendingTasks = getTask('pending');
//   const completedTasks = getTask('completed');
//   const overdueTasks = getTask('overdue');
...
export default function Dashboard() {
  ...
  // Load data on mount
  useEffect(() => {
    void listTasks({ sort: 'createdAt', order: 'desc' });
    void fetchSummary();
  }, [listTasks, fetchSummary]);

  // --- Derived stats (fallback to client-side computation when summary not yet loaded) ---
  const now = useMemo(() => new Date(), []);

  const pendingTasks = useMemo(
    () => tasks.filter(t => (t.status === 'pending' || t.status !== 'completed')),
    [tasks]
  );
  ...
  const recentTasks = useMemo(() => {
    ...
    return [...list].sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()).slice(0, 5);
        ...
        <Card className="hover-glow smooth-transition">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              ...
              <div className="text-center">
                <p className="text-2xl font-bold text-secondary">{inProgress}</p>
                <p className="text-sm text-muted-foreground">Remaining</p>
              </div>
            </div>
          </CardContent>
        ...
        <CardContent>
          <div className="space-y-3">
            {recentTasks.map((task) => (
              <div key={task._id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border">
                <div className={`w-2 h-2 rounded-full ${
                  task.status === 'completed' ? 'bg-success' : 'bg-secondary'
                }`}></div>
                <div className="flex-1">
